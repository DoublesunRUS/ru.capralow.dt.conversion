...
import com._rarus.dt.conversion.plugin.ui.model.Model;
...
public class MovingBoxView extends ViewPart {
	...
	protected ViewerComparator booksBoxesGamesSorter, noArticleSorter;
	...
	protected void createActions() {
		...
		addBookAction.setImageDescriptor(Activator.getImageDescriptor("newBook.gif"));
		...
		removeAction.setImageDescriptor(Activator.getImageDescriptor("remove.gif"));		
	}
	...
	protected void updateSorter(Action action) {
		if(action == booksBoxesGamesAction) {
			...
			if(action.isChecked()) {
				treeViewer.setComparator(booksBoxesGamesSorter);
			} else {
			...
			if(action.isChecked()) {
				treeViewer.setComparator(noArticleSorter);
			} else {
				treeViewer.setComparator(null);
			}
		}
