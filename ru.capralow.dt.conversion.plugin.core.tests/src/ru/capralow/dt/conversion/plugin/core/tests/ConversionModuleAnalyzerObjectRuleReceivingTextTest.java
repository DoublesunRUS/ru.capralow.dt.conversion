...
public class ConversionModuleAnalyzerObjectRuleReceivingTextTest {
	...
	public void testConfigurationFormatObject() {
		String report1 = String.join(LS, "#Область ПКОПолученияОбъектКонфигурацииФормата",
				"Процедура ДобавитьПКО_ПКОПолученияОбъектКонфигурацииФормата(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияОбъектКонфигурацииФормата\";",
				FILLED_CONFIGURATION_OBJECT, FILLED_FORMAT_OBJECT, IDENTIFICATION_UUID, "	", "", END_OF_PROCEDURE,
				END_OF_REGION);
	...
	public void testConfigurationFormatObjectWithFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияОбъектКонфигурацииФорматаСПолями",
				"Процедура ДобавитьПКО_ПКОПолученияОбъектКонфигурацииФорматаСПолями(ПравилаКонвертации)", "", NEW_RULE,
	...
	public void testConfigurationObject() {
		String report1 = String.join(LS, "#Область ПКОПолученияОбъектКонфигурации",
				"Процедура ДобавитьПКО_ПКОПолученияОбъектКонфигурации(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияОбъектКонфигурации\";",
				FILLED_CONFIGURATION_OBJECT, EMPTY_FORMAT_OBJECT, IDENTIFICATION_UUID, "	", "", END_OF_PROCEDURE,
				END_OF_REGION);
	...
	public void testConfigurationObjectWithFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияОбъектКонфигурацииСПолями",
				"Процедура ДобавитьПКО_ПКОПолученияОбъектКонфигурацииСПолями(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияОбъектКонфигурацииСПолями\";",
				FILLED_CONFIGURATION_OBJECT, EMPTY_FORMAT_OBJECT, IDENTIFICATION_UUID, "	", NEW_HEADER,
				HEADER_SECTION, "", END_OF_PROCEDURE, END_OF_REGION);
	...
	public void testFormatObject() {
		String report1 = String.join(LS, "#Область ПКОПолученияОбъектФормата",
				...
				EMPTY_CONFIGURATION_OBJECT, FILLED_FORMAT_OBJECT, IDENTIFICATION_UUID, "	", "", END_OF_PROCEDURE,
				END_OF_REGION);
	...
	public void testFormatObjectWithFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияОбъектФорматаСПолями",
				"Процедура ДобавитьПКО_ПКОПолученияОбъектФорматаСПолями(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияОбъектФорматаСПолями\";",
				EMPTY_CONFIGURATION_OBJECT, FILLED_FORMAT_OBJECT, IDENTIFICATION_UUID, "	", NEW_HEADER,
				HEADER_SECTION, "", END_OF_PROCEDURE, END_OF_REGION);
		...
		assertEquals("Формирование модуля обмена: Добавить ПКО для получения c объектом формата и полями", report1,
				report2.toString());
	}
	...
	public void testHeaderAndTabularSection() {
		String report1 = String.join(LS, "#Область ПКОПолученияШапкаТЧ",
				"Процедура ДобавитьПКО_ПКОПолученияШапкаТЧ(ПравилаКонвертации)", "", NEW_RULE,
	...
	public void testIdentificationVariant1WithFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияИдентификация1СПолями",
				"Процедура ДобавитьПКО_ПКОПолученияИдентификация1СПолями(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияИдентификация1СПолями\";",
				FILLED_CONFIGURATION_OBJECT, EMPTY_FORMAT_OBJECT, IDENTIFICATION_UUID, "	", "",
				IDENTIFICATION_FIELDS2, END_OF_PROCEDURE, END_OF_REGION);
		...
		assertEquals("Формирование модуля обмена: Добавить ПКО для получения по УУИД с полями", report1,
				report2.toString());
	}
	...
	public void testIdentificationVariant2NoFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияИдентификация2",
				"Процедура ДобавитьПКО_ПКОПолученияИдентификация2(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияИдентификация2\";",
				EMPTY_CONFIGURATION_OBJECT, EMPTY_FORMAT_OBJECT, IDENTIFICATION_SEARCH_FIELDS, "	", "",
		...
		assertEquals("Формирование модуля обмена: Добавить ПКО для получения по Полям поиска без полей", report1,
				report2.toString());
	}
	...
	public void testIdentificationVariant2WithFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияИдентификация2СПолями",
				"Процедура ДобавитьПКО_ПКОПолученияИдентификация2СПолями(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияИдентификация2СПолями\";",
				EMPTY_CONFIGURATION_OBJECT, EMPTY_FORMAT_OBJECT, IDENTIFICATION_SEARCH_FIELDS, "	", "",
				IDENTIFICATION_FIELDS1, END_OF_PROCEDURE, END_OF_REGION);
		...
		assertEquals("Формирование модуля обмена: Добавить ПКО для получения по Полям поиска с полями", report1,
				report2.toString());
	}
	...
	public void testIdentificationVariant3NoFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияИдентификация3",
				"Процедура ДобавитьПКО_ПКОПолученияИдентификация3(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияИдентификация3\";",
				EMPTY_CONFIGURATION_OBJECT, EMPTY_FORMAT_OBJECT, IDENTIFICATION_UUID_THEN_SEARCH_FIELDS, "	", "",
				END_OF_PROCEDURE, END_OF_REGION);
		...
		assertEquals("Формирование модуля обмена: Добавить ПКО для получения по УУИД и Полям поиска без полей", report1,
				report2.toString());
	}
	...
	public void testIdentificationVariant3WithFields() {
		String report1 = String.join(LS, "#Область ПКОПолученияИдентификация3СПолями",
				"Процедура ДобавитьПКО_ПКОПолученияИдентификация3СПолями(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияИдентификация3СПолями\";",
				FILLED_CONFIGURATION_OBJECT, EMPTY_FORMAT_OBJECT, IDENTIFICATION_UUID_THEN_SEARCH_FIELDS, "	", "",
				IDENTIFICATION_FIELDS2, END_OF_PROCEDURE, END_OF_REGION);
		...
		assertEquals("Формирование модуля обмена: Добавить ПКО для получения по УУИД и Полям поиска с полями", report1,
				report2.toString());
	}
	...
	public void testNameOnly() {
		String report1 = String.join(LS, "#Область МинимальноеПКОПолучения",
				"Процедура ДобавитьПКО_МинимальноеПКОПолучения(ПравилаКонвертации)", "", NEW_RULE,
	...
	public void testTwoTabularSections() {
		String report1 = String.join(LS, "#Область ПКОПолученияДвеТЧ",
				"Процедура ДобавитьПКО_ПКОПолученияДвеТЧ(ПравилаКонвертации)", "", NEW_RULE,
				"	ПравилоКонвертации.ИмяПКО                       = \"ПКОПолученияДвеТЧ\";",
				FILLED_CONFIGURATION_OBJECT, FILLED_FORMAT_OBJECT, IDENTIFICATION_UUID, "	", "	",
				FIRST_TABULAR_SECTION, "	", SECOND_TABULAR_SECTION, "", END_OF_PROCEDURE, END_OF_REGION);
	...
	public void testWEvent12() {
		String report1 = String.join(LS, "#Область ПКОПолученияСобытие12",
				...
				"Процедура ПКО_ПКОПолученияСобытие12".concat(ON_RECEIVING_EVENT_SECTION), END_OF_REGION);

		StringBuilder report2 = new StringBuilder();
		CmObjectRule objectRule = ConversionModuleAnalyzerUtils.addFilledObjectRule("ПКОПолученияСобытие12", false,
				false, false, true, false);
		ConversionModuleAnalyzerUtils.addEvents(false, true, true, false, objectRule, null);
