package ru.capralow.dt.conversion.plugin.ui.conversionmodule;
...
import ru.capralow.dt.conversion.plugin.core.analyzer.EnterpriseDataAnalyzer;
import ru.capralow.dt.conversion.plugin.core.analyzer.ExchangeProjectsAnalyzer;
...
public class ConversionModuleXtextBuilderParticipant implements org.eclipse.xtext.builder.IXtextBuilderParticipant {
	...
	public void build(IBuildContext context, IProgressMonitor monitor) throws CoreException {
		...
		URI xmiURI = ExchangeProjectsAnalyzer.getResourceURIforPlugin(project);
		...
		for (EpFormatVersion epFormatVersion : exchangeProject.getFormatVersions()) {
			xmiURI = EnterpriseDataAnalyzer.getResourceURIforPlugin(epFormatVersion.getVersion(), project);
		...
		for (CommonModule commonModule : exchangeProject.getFormatModules()) {
			xmiURI = ConversionModuleAnalyzer.getResourceURIforPlugin(commonModule.getName(), project);
